...
\begin{document}
	...
	\begin{itemize}
		...
		\end{verbatim}
	\end{itemize}\par
	Hiện tại gói lệnh \verb|set_box| có sẵn $29$, được liệt kê ở mục số $3$. Trong đó, kiểu số 0 là kiểu hiển thị không có khung. Đối với các \verb|theorem| mà thầy cô không muốn tạo khung thì hãy dùng kiểu số $0$ \textit{(đây là bước bắt buộc)}.
	\newpage
	...
	\begin{center}
		\begin{tabular}{|m{5cm}|m{4.5cm}|m{5.8cm}|}
			...
			\\\hline
			{\color{blue}\verb|\sb@title|}&tiêu đề sẽ hiển thị ở đầu đoạn nội dung (nếu có)&
			\\\hline
		\end{tabular}
	\end{center}\par
	Đối với khung mà tiêu đề \textbf{không nằm} ở đầu đoạn của nội dung, thầy cô nên sử dụng {\color{blue}\verb|\sb@labelthmshort|} hoặc {\color{blue}\verb|\sb@labelthmfull|} để không hiện dấu chấm. Đồng thời, ta khai báo {\color{blue}\verb|\def\sb@title{}|}.\par
	Đối với khung mà tiêu đề \textbf{nằm} ở đầu đoạn của nội dung, thầy cô nên sử dụng\\
	{\color{blue}\verb|\sb@labelthmshortDot|} hoặc {\color{blue}\verb|\sb@labelthmfullDot|} để hiện dấu chấm. Đồng thời, ta khai báo đầy đủ {\color{blue}\verb|\def\sb@title{<Định dạng hiển thị>}|}\par
	Khai báo {\color{blue}\verb|\sb@title|} đầy đủ để giúp lệnh \verb|\immini| và \verb|\sochc| có thể hoạt động đúng.\par
	Thầy cô khai báo theo cấu trúc như sau, tên command thầy cô đặt tuỳ ý.
	\begin{verbatim}
		\makeatletter
		\newcommand{\bgBOXnew}{%
				\def\sb@title{<Định dạng>}
				\def\sb@beginbox{
						\begin{tcolorbox}[%
								<Định dạng>
								]
						}
				\def\endbox{<Định dạng>\end{tcolorbox}}
				%%% Nếu trong tcolorbox khác thì sẽ mất khung.
				\ifsb@InTcolorbox
						\Ifsb@hideboxInTcb
								\def\sb@title{<Định dạng>}
								\def\sb@beginbox{<Định dạng>}
								\def\endbox{<Định dạng>}
						\fi
				\fi
		}
	\makeatother
	\end{verbatim}
	Chẳng hạn, \verb|set_box| đã khai báo sẵn lệnh như sau dựa trên \verb|\bgBOX| của gói \verb|ex_test|:
	\begin{verbatim}
		\makeatletter
		\newcommand{\bgBOXnew}{%
				\def\sb@title{}
				...
				\def\endbox{\par\noindent\hfill\sb@sublabelthmBracket\end{tcolorbox}}
				%%% Nếu trong tcolorbox khác thì sẽ mất khung.
				\ifsb@InTcolorbox
						\Ifsb@hideboxInTcb
								\def\sb@title{\par\noindent{\bf\color{red!30!black}
								\fontfamily{qag}\selectfont\sb@labelthmfullDot~}}
								\def\sb@beginbox{}
								\def\endbox{\par\noindent\hfill\sb@sublabelthmBracket\par}
						\fi
				\fi
		}
	\end{verbatim}
	Thầy cô tạo khung tương tự với cách dùng số:
	\begin{verbatim}
		% Đã định dạng in đậm và màu \bgBOXnew
		\setTheoBox*{nx}{\bgBOXnew}{Nhận xét}
	\end{verbatim}
	Ta thu được kết quả như sau khi Nhận xét không nằm độc lập với Định nghĩa:
	\begin{dn}
		Nội dung định nghĩa...
	\end{dn}
	\begin{nx}[Tên nhận xét]
		Nội dung nhận xét...
	\end{nx}
	Ta thu được kết quả như sau khi Nhận xét nằm trong Định nghĩa, chi tiết xem mục 2.3 ở phía sau:
	\begin{dn}
		Nội dung định nghĩa...
		\begin{nx}[Tên nhận xét]
			Nội dung nhận xét...
