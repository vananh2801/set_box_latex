...
\begin{document}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item
			{\color{blue}\verb|\setTheoBox{<tên theorem>}{<kiểu>}{<Tiêu đề>}|}: tạo khung đánh số.
			\item
			{\color{blue}\verb|\setTheoBox*{<tên theorem>}{<kiểu>}{<Tiêu đề>}|}: tạo khung không đánh số.
		\end{itemize}
		Chẳng hạn:
		\begin{verbatim}
		...
		\end{verbatim}
	\end{itemize}\par
	Hiện tại gói lệnh \verb|set_box| có sẵn $30$, được liệt kê ở mục $3$. Trong đó, kiểu số 0 là kiểu hiển thị không có khung. Đối với các \verb|theorem| mà thầy cô không muốn tạo khung thì hãy dùng kiểu số $0$ \textit{(đây là bước bắt buộc)}.
	\newpage
	...
	Gói lệnh \verb|set_box| có hỗ trợ khung riêng do thầy cô tự khai báo. Ta cần chú ý các lệnh sau:
	\begin{center}
		\begin{tabular}{|m{5cm}|m{4.5cm}|m{5.8cm}|}
			\hline
			\hfil\textbf{Lệnh}&\hfil\textbf{Giải thích}&\hfil\textbf{Minh hoạ}
			\\\hline
			{\color{blue}\verb|\sb@labelthm|}&tiêu đề&\textbf{Định nghĩa}
			\\\hline
			{\color{blue}\verb|\sb@Currentlabel|}&số thứ tự&\textbf{1}
			\\\hline
			{\color{blue}\verb|\sb@sublabelthm|}&tiêu đề phụ&\textbf{Tên định nghĩa}
			\\\hline
			{\color{blue}\verb|\sb@sublabelthmBracket|}&tiêu đề phụ trong ngoặc&\textbf{(Tên định nghĩa)}
			\\\hline
			{\color{blue}\verb|\sb@labelthmshort|}&tiêu đề + số thứ tự&\textbf{Định nghĩa 1}
			\\\hline
			{\color{blue}\verb|\sb@labelthmfull|}&tiêu đề + số thứ tự + tiêu đề phụ&\textbf{Định nghĩa 1 (Tên tiêu đề)}
			\\\hline
			{\color{blue}\verb|\sb@labelthmshortDot|}&tiêu đề + số thứ tự + dấu chấm&\textbf{Định nghĩa 1.}
			\\\hline
			{\color{blue}\verb|\sb@labelthmfullDot|}&tiêu đề + số thứ tự + tiêu đề phụ + dấu chấm&\textbf{Định nghĩa 1 (Tên tiêu đề).}
			\\\hline
		\end{tabular}
	\end{center}\par
	Thầy cô khai báo kiểu khung riêng nằm ngoài \verb|set_box|. Vì trong khai báo có chứa dấu a còng \verb|@| nên ta phải khai báo trong cặp lệnh \verb|\makeatletter| và \verb|\makeatother|. Sau đây là cấu trúc chuẩn dùng cho gói \verb|set_box|:
	\begin{verbatim}
		\makeatletter
		\newcommand{<tên kiểu>}{%
				%%% Khai báo khung.
	...
	\newpage
	Ở mỗi phần này, chúng ta để phải khai báo đầy đủ 3 \verb|macro| như sau:
	\begin{itemize}
		\item {\color{blue}\verb|\sb@title|}: Tiêu đề xuất hiện ở đầu đoạn nội dung.
		\begin{itemize}
			\item Khi ta cần hiện tiêu đề nằm trên khung, thì ta phải ẩn tiêu đề ở đầu đoạn nội dung. Ta khai báo {\color{blue}\verb|\def\sb@title{}|}.
			\item Ngược lại, ta cần hiện tiêu đề ở đầu đoạn nội dung. Ta khai báo\\
			{\color{blue}\verb|\def\sb@title{\sb@labelthmfullDot}|}\\
			hoặc {\color{blue}\verb|\def\sb@title{\sb@labelthmshortDot}|}
			\item Khai báo {\color{blue}\verb|\sb@title|} đầy đủ để giúp lệnh \verb|\immini| và \verb|\sochc| có thể hoạt động đúng.
		\end{itemize}
		\item {\color{blue}\verb|\sb@beginbox|}: 
		\begin{itemize}
			\item Ta khai báo định dạng \verb|tcolorbox| trong cặp $[...]$ của lệnh \verb|\begin{tcolorbox}|.
			\item Nếu muốn mất khung như kiểu số $0$ thì ta khai báo {\color{blue}\verb|\def\sb@beginbox{}|}
		\end{itemize}
		\item {\color{blue}\verb|\endbox|}:
		\begin{itemize}
			\item Nếu ta đã có dùng \verb|\begin{tcolorbox}| thì \textbf{bắt buộc} phải có \verb|\end{tcolorbox}| ở cuối nội dung khai báo {\color{blue}\verb|\def\endbox{...}|}. Nếu ta không cần có tiêu đề phụ ở cuối khung thì ta chỉ cần khai báo {\color{blue}\verb|\def\endbox{\end{tcolorbox}}|} là được. Ngược lại ta cần hiện tiêu đề phụ ở cuối (thường dùng để ghi nguồn ví dụ, bài tập,...) thì ta có thể khai báo\\
			{\color{blue}\verb|\def\endbox{\par\noindent\hfill|\\
			\verb|\textit{\sb@sublabelthmBracket}\end{tcolorbox}}|}
			\item Nếu không khai báo \verb|\begin{tcolorbox}| thì ta không cần \verb|\end{tcolorbox}|. Làm tương tự như trên nhưng không chèn \verb|\end{tcolorbox}|. Chẳng hạn, {\color{blue}\verb|\def\endbox{}|} hoặc \\
			{\color{blue}\verb|\def\endbox{\par\noindent\hfill|\verb|\textit{\sb@sublabelthmBracket}}|}
		\end{itemize}
	\end{itemize}\par
	Gói \verb|set_box| đã khai báo sẵn lệnh {\color{blue}\verb|\bgBOXnew|} dựa trên {\color{blue}\verb|\bgBOX|} của gói \verb|ex_test|. Ngoài ra thầy cô có thể tham khảo cách khai báo bên trong \verb|set_box.sty|.
	\begin{verbatim}
		\makeatletter
		\newcommand{\bgBOXnew}{%
				\def\sb@title{}
